...
class AlexaTop(object):
  def __init__(self, access_key_id, secret_accessss_key):
    super(AlexaTop,self).__init__()

    self.access_key_id = access_key_id
    self.secret_access_key = secret_accessss_key

  def get_sites(self, country_code, start=1, count=1):

    time_stamp = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    date_stamp = datetime.utcnow().strftime("%Y%m%d")

    ats = AlexaTopSites(self.access_key_id, self.secret_access_key)

    for i in range(int(count / AWS_MAX_COUNT)):
      ats.get_sites(start, AWS_MAX_COUNT, country_code, time_stamp, date_stamp)
      start += AWS_MAX_COUNT

    return ats.get_sites(start, count % AWS_MAX_COUNT, country_code, time_stamp, date_stamp)
